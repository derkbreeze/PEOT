...
def asot_objective(T, cost_matrix, eps, alpha, radius, ub_frames, ub_actions,
    ...
    frames_ub_penalty = lambda_frames * kld(frames_marg, dx)
    actions_marg = T_mask.sum(dim=1)
    actions_ub_penalty = lambda_actions * kld(actions_marg, dy)
...
def get_m_matrix(centers, half_widths, T, sharpness = 2.5):
    device = centers.device
    centers, half_widths = centers.unsqueeze(1), half_widths.unsqueeze(1)
    the_range = torch.arange(0, T, dtype=torch.float32).to(device)
    ...
    M_prime = -1 * (denom_1_prime + denom_2_prime)
    M = torch.exp(M_prime)  # (M x T)
    return M

length_logits = torch.randn(K, requires_grad = True)
energy_optimizer = torch.optim.Adam([length_logits], lr = 0.1)
energy_list = []
for num_iter in range(50):
    length_probs = F.softmax(length_logits)
    length_probs = length_probs[transcript]
    entropy = -(torch.log(length_probs + 1e-7) * (length_probs + 1e-7)).sum()
    
    lengths = length_probs * T
    half_widths = lengths / 2
    centers = torch.cumsum(lengths, 0) - half_widths
    M = get_m_matrix(centers, half_widths, T, sharpness = 2.5)

    observation_energy = -(log_probs.T * M).sum() / T
    pred_lengths = length_probs * T
    #length_energy = (pred_lengths - mean_lengths).abs().sum() / b
    length_energy = (pred_lengths - torch.from_numpy(buffer.mean_lengths)).abs().sum() / b
    energy = observation_energy + beta * length_energy
