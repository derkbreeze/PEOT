...
def asot_objective(T, cost_matrix, eps, alpha, radius, ub_frames, ub_actions, lambda_frames, lambda_actions, mask=None):
    dev = cost_matrix.device
    B, N, K = cost_matrix.shape
...
def segment_asot(cost_matrix, mask=None, eps=0.07, alpha=0.3, radius=0.04, ub_frames=False, ub_actions=True, 
                 lambda_frames=0.1, lambda_actions=0.05, n_iters=(25, 1), stable_thres=7., step_size=None, init=False):
    ...
    if init:
        T = torch.zeros((B, N, K), device=dev)
        label_greedy = cost_matrix.argmin(-1).squeeze()
        for t in range(N):
            label = label_greedy[t]
            T[:, t, label] = 1
        T /= T.sum()
    ...
    while True:
        with torch.no_grad():
            obj = asot_objective(T, cost_matrix, eps, alpha, radius, ub_frames, ub_actions, lambda_frames, lambda_actions, mask=mask)
        ...
        if not ub_frames and not ub_actions:
            T = project_to_polytope_KL(fgw_cost_matrix, mask, eps, dx, dy, n_iters=n_iters[1],stable_thres=stable_thres)
        elif not ub_frames:
