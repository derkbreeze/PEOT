...
def pred_to_gt_match(pred_labels, gt_labels):
    ...
    for pred_idx, pred_lab in enumerate(pred_uniq):
        for gt_idx, gt_lab in enumerate(gt_uniq):
            affinity_labels[pred_idx, gt_idx] = np.logical_and(pred_labels == pred_lab, gt_labels == gt_lab).sum()
...
def eval_f1(pred_labels, gt_labels, n_videos, exclude_cls=None, pred_to_gt=None, n_sample=15, n_exper=50, eps=1e-8):
    ...
    segments_count_ = np.sum(gt_labels_[:-1] != gt_labels_[1:]) + 1
    assert segments_count_ == segments_count
    ...
    #import ipdb;ipdb.set_trace()
    return_stats = {}
    return_stats['precision'] = [tp_agg / n_exper, n_videos * n_actions]
    return_stats['recall'] = [tp_agg / n_exper, segments_count]
    return_stats['f1'] = f1

    return return_stats, pred_to_gt
...
def indep_eval_metrics(pred_labels_batch, gt_labels_batch, mask, metrics=['mof', 'f1', 'miou'], exclude_cls=None, pred_to_gt=None):
    ...
    values = {'mof':0, 'f1':0, 'f1_macro':0, 'miou':0}
    ...
    for b in range(B):
        ...
        for metric in metrics:
            ...
            #     pred_labels = pred_labels_batch.squeeze().cpu().clone()
            #     for i in range(pred_labels_batch.size(1)):
            #         key = int(pred_labels_batch[0][i])
            #         if key in p2gt_local.keys():
            #             pred_labels[i] = p2gt_local[key]

            #     f1_macro = sklearn.metrics.f1_score(gt_labels_batch.squeeze().cpu(), pred_labels, average='macro')
            #     values['f1_macro'] = f1_macro
            #     import ipdb;ipdb.set_trace()

            if metric == 'f1':
                values[metric] += score['f1'] / B
            else:
